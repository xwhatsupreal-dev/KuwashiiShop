...
export const GachaResultModal: React.FC<GachaResultModalProps> = ({
  ...
  const allSalt = result ? result.drops.every((d) => d.isSalt) : false;
      ...
      {isOpen && result && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          >
            ...
            <div
              className={`absolute top-0 right-0 w-48 h-48 ${allSalt ? "bg-zinc-500/10" : "bg-amber-500/15"} blur-[60px] pointer-events-none rounded-full`}
            />
            <div
              className={`absolute bottom-0 left-0 w-48 h-48 ${allSalt ? "bg-zinc-500/5" : "bg-orange-600/10"} blur-[60px] pointer-events-none rounded-full`}
            />
            ...
            <div className="mb-8 relative mt-4">
              ...
              >
                <div
                  className={`absolute inset-2 rounded-full ${allSalt ? "bg-zinc-900" : "bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center"} shadow-inner`}
                >
                  {allSalt ? (
            ...
            <h2
              className={`text-2xl sm:text-3xl font-bold mb-3 font-display tracking-wide ${allSalt ? "text-zinc-300" : "bg-gradient-to-r from-amber-300 via-yellow-400 to-orange-500 bg-clip-text text-transparent"}`}
            >
            ...
            <p className="text-zinc-400 text-sm mb-2 max-w-sm mx-auto font-medium leading-relaxed">
              จากการเปิด {result.item.name} จำนวน{" "}
              <span className="text-white font-bold">
                {result.purchaseQty || result.drops.length}
              </span>{" "}
              ครั้ง
            </p>
            ...
            {result.remainingStock !== undefined ? (
              <div className="mb-6 flex justify-center">
                <span className="text-emerald-400 font-medium text-xs bg-emerald-500/10 py-1.5 px-4 rounded-full border border-emerald-500/20 tracking-wide">
                  กล่องสุ่มเหลือ{" "}
                  <span className="font-bold">{result.remainingStock}</span>{" "}
                  ชิ้น
                </span>
              </div>
            ) : (
                  ...
                  .map((drop: any, idx) => (
                    ...
                    >
                      <div
                        ...
                        style={{
                          backgroundColor: drop.isSalt
                            ? "rgba(255,255,255,0.05)"
                            : drop.color
                              ? `${drop.color}20`
                              : "rgba(245,158,11,0.2)",
                          border: `1px solid ${drop.isSalt ? "rgba(255,255,255,0.1)" : drop.color ? `${drop.color}50` : "rgba(245,158,11,0.5)"}`,
                        }}
                      >
                        {drop.isSalt ? "🧂" : "✨"}
                      </div>
                      <div className="flex-1 text-left flex justify-between items-center min-w-0">
                        <div className="pr-3">
                          <div
                            className={`${drop.isSalt ? "text-zinc-300" : "text-white"} font-bold text-[15px] sm:text-base leading-snug`}
                          >
                            {drop.name}
                          </div>
